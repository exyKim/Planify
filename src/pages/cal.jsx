...
function Calendar() {
  ...
  const getMonday = (date) => {
    ...
    monday.setDate(d.getDate() - ((day + 6) % 7)); // 월=1 보정
  ...
  const handleModalSubmit = (data) => {
    ...
    const targetDate = parseLocalYmd(dateKey);   // ❌ 여기서 Date로 변환하면서 밀림 발생
    ...
    <div className="calendar-container">
      ...
      <main className="main-content">
        <header className="calendar-header">
          <div className="date-navigation">
            <button className="nav-btn" onClick={() => navigateWeek(-1)}>
              <img src="/src/images/nav_btn_l.svg" alt="Previous" />
            </button>
            ...
            <button className="nav-btn" onClick={() => navigateWeek(1)}>
              <img src="/src/images/nav_btn_r.svg" alt="Next" />
            </button>
          </div>
                ...
                <div key={day} className="weekday-item">
                  ...
                  <div className="timeline">
                    {daySchedules.map((schedule, idx) => (
                      <div
                        key={`${dateKey}-${idx}`}
                        className={`schedule-item ${schedule.type} ${schedule.highlight ? 'highlight' : ''}`}
                        onDoubleClick={() => handleToggleHighlight(dateKey, idx)}
        ...
        <div className="todo-date-display">
          <div className="todo-date-left">{currentDate.getDate()}</div>
          <div className="todo-date-right">
            <div>{currentDate.getFullYear()}</div>
            <div>{currentDate.toLocaleString("en-US", { month: "short" })}.</div>
          </div>
        </div>
        ...
        <div className="todo-section">
          <div className="todo-scroll">
            <TodoList
              items={(schedules[formatDateKey(currentDate)] || []).map((item) => ({
